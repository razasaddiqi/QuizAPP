...
int count=0;
  ...
  buttonFunction(id)  async{
    var d = await Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => QuizApp2(id,quizBrain,selected_answers[id])));
    // print(d);
    if(d[0]){
      setState(() {
      ...
      int index=wrong_answers.indexWhere((i) => i.toString().contains(wrong_answers2[id].toString()));
      if (index>-1){
  ...
  void checkAnswer(int userPickedAnswer,BuildContext context) {
    ...
    setState(() {
      if (quizBrain.isFinished() == true) {
        ...
        Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => SecondRoute(quizBrain,correct,wrong,correct_answers,wrong_answers,timer)));
      }
      else {
